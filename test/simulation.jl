	...
	@testset "output dimenions" begin
		for subject ∈ ["single", "multi"]
			if subject == "multi"
				...
				design = SingleSubjectDesign(; conditions = Dict(:a => ["a"])) |> x -> RepeatDesign(x, 7)
			...
			for channel ∈ ["single", "multi"]
				...
				for sim_onset ∈ ["noonset", "yesonset"]
					...
					for return_epoched ∈ [false, true]
						...
						try
							data, events = simulate(MersenneTwister(1), simulation; return_epoched = return_epoched)
